...
public class MainActivity extends AppCompatActivity {
    ArrayAdapter<String> arrayAdapter; //not used after all
    List<WeatherForecastItem> forecastList;
     ...
     * This function will get called (on the main UI thread) once we have successfully returned
     * from the API with results.
     * @param forecast The results of the API call. If an error occurred, this will be null.
     */
    void handleWeatherForecastResult(WeatherForecast forecast) {
        Log.d("MainActivity", "Back from API on the UI thread with the weather Forecast!");
        ...
        if (forecast == null) {
            ...
            // Inform the user
            Toast.makeText(this, "An error occurred when retrieving the forecast",
                    Toast.LENGTH_LONG).show();
        } else {
            Log.d("MainActivity", "Forecast: " + "PLACEHOLDER");
            forecastList = forecast.getForecastItems();
            ListView mListView = (ListView) findViewById(R.id.listView);

            //custom adapter to avoid having to create a list of strings
            ForecastListAdapter adapter = new ForecastListAdapter(this, R.layout.adapter_view_layout, forecastList );
